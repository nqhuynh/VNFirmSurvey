...
* label the 2000 ownership codes
...
* merge m:1 madn using "/home/nqh3/project/VNFirmSurvey/inst/extdata/Stata_2004/\2001\data\2001 firms to remove.dta"
* keep if _merge==1
* drop _merge
...
* export the dataset
sort madn year
save "$inconsis\Firms with at least one invalid VSIC2007 industry code", replace

* make the suggested fixes to VSIC2007 industry codes
use "$output\Master enterprise dataset 19.dta", clear
merge m:1 year madn using "$inconsis\Firms with at least one invalid VSIC2007 industry code - suggested fixes.dta"
...
merge m:1 province2003 using "$output\Province codes 2003 to 2004.dta", update
drop _merge
merge m:1 province2004 using "$output\Province codes 2003 to 2004.dta", update
drop _merge
...
* or later
merge m:1 vsic2007 using "$industry\VSIC 2007 - VSIC 1993 conversion - revised (English).dta"
drop if _merge==2
drop _merge
...
merge m:1 isic4 using "$data\2001 US AVE tariffs 4-digit ISIC industry weight all.dta"
drop if _merge==2
drop _merge
...
merge m:1 isic4 year using "$tarexp\Vietnam's MFN tariffs 2002 to 2017 4-digit ISIC revision 3.dta"
...
merge m:1 isic3 using "$data\2001 US AVE tariffs 3-digit ISIC industry.dta", keepusing(col2_ave_all mfn_ave_all)
drop if _merge==2
drop _merge
...
merge m:1 province using "$output\Median distance from major seaport.dta"
drop if _merge==2
drop _merge

* merge with provincial tariffs from McCaig (2011) JIE
merge m:1 province using "$output\Provincial tariffs (McCaig JIE 2011).dta"
drop if _merge==2
drop _merge
